...
def is_shorts(youtube_id: str) -> bool:
    ...
    response = requests.head(
        shorts_url, cookies=cookies, headers=requests_headers(), timeout=10, proxies= get_proxies()
    )
...
def is_shorts(youtube_id: str) -> bool:
    ...
    cookies = {"SOCS": "CAI"}
    response = requests.head(
        shorts_url, cookies=cookies, headers=requests_headers(), timeout=10, proxies= get
    )
...
def get_proxies():
    proxy = EnvironmentSettings.HTTP_PROXY
